...
/*   Updated: 2022/06/27 18:55:33 by fathjami         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	rotate(t_program *game_ptr, double angle)
{
	...
	game_ptr->dir_x = game_ptr->dir_x * cos(angle) - game_ptr->dir_y * sin(angle);
	game_ptr->dir_y = olddirx * sin(angle) + game_ptr->dir_y * cos(angle);
	game_ptr->plane_x = game_ptr->plane_x * cos(angle) - game_ptr->plane_y * sin(angle);
	game_ptr->plane_y = oldplanx * sin(angle) + game_ptr->plane_y * cos (angle);
}

void forword(t_program * game_ptr)
{
	if (game_ptr->map.map[(int) (game_ptr->pos_x + game_ptr->dir_x )]\
		[(int) (game_ptr->pos_y + game_ptr->dir_y)] == '0')
	{
...
{
	if (game_ptr->map.map[(int) (game_ptr->pos_x)]\
		[(int) (game_ptr->pos_y +  game_ptr->dir_y + game_ptr->plane_y)] == '0')
	{
